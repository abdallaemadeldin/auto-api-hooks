...
Every generated file includes an auto-generated header with a timestamp so you can tell at a glance which files are machine-generated and when they were last regenerated:
...
- **Circular reference handling** -- Types that reference themselves (directly or mutually) are automatically wrapped with `z.lazy()` to prevent runtime errors
...
For circular types (e.g. a `Category` with a `parent` field referencing itself), the generator detects cycles and emits:
...
**Does it handle circular type references in Zod schemas?**
Yes. The generator automatically detects self-referencing and mutually recursive types and wraps them with `z.lazy()` to prevent runtime errors.

**What happens if two operations produce the same hook name?**
The generator warns you at build time with details about which operations collided, so you can fix the `operationId` values in your spec before it causes issues.
