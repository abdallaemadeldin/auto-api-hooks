...
describe('CLI', () => {
  ...
  it('previews files with --dry-run without writing', () => {
    const result = execSync(
      `npx tsx ${cliPath} generate --spec ${specPath} --fetcher fetch --output ${tmpDir} --dry-run`,
      { cwd, encoding: 'utf-8' }
    )

    // Should list files in output
    expect(result).toContain('types.ts')
    expect(result).toContain('client.ts')
    expect(result).toContain('index.ts')

    // But no files should be written
    expect(fs.existsSync(path.join(tmpDir, 'types.ts'))).toBe(false)
    expect(fs.existsSync(path.join(tmpDir, 'client.ts'))).toBe(false)
  })

  it('cleans stale files with --clean flag', () => {
    ...
    // Create a fake stale file that looks auto-generated
    const staleFile = path.join(tmpDir, 'old-hook.ts')
    fs.writeFileSync(staleFile, '// Auto-generated by auto-api-hooks — do not edit manually.\nexport function stale() {}')
    expect(fs.existsSync(staleFile)).toBe(true)

    // Regenerate with --clean
    execSync(
      `npx tsx ${cliPath} generate --spec ${specPath} --fetcher fetch --output ${tmpDir} --clean`,
      { cwd, encoding: 'utf-8' }
    )
